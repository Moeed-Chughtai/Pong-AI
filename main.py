    ...
    def train_ai(self, genome1, genome2, config, draw=False):
        ...
        while run:
            ...
            duration = time.time() - start_time
             ...
             # End game between 2 genomes if one scores a point or max hits are achieved
    ...
    def move_ai_paddles(self, net1, net2):
        ...
        for (genome, net, paddle, left) in players:
            ...
            if decision == 0:  # Don't move
                genome.fitness -= 0.01  # To discourage this
            elif decision == 1:  # Move up
                valid = self.game.move_paddle(left=left, up=True)
            else:  # Move down
                valid = self.game.move_paddle(left=left, up=False)

            if not valid:  # If the movement makes paddle go off screen, discourage it
                genome.fitness -= 1
...
def eval_genomes(genomes, config):
    """
    Run each genome against each other genome once to determine the fitness
    """
    width, height = 700, 500
    win = pygame.display.set_mode((width, height))
    pygame.display.set_caption("Pong")

    # genome_id not required
    for i, (genome_id1, genome1) in enumerate(genomes):
        print(round(i/len(genomes) * 100), end=" ")
        genome1.fitness = 0
        # Prevent genomes from playing against each other multiple times
        for genome_id2, genome2 in genomes[min(i+1, len(genomes) - 1):]:
            genome2.fitness = 0 if genome2.fitness == None else genome2.fitness
            pong = PongGame(win, width, height)
            pong.train_ai(genome1, genome2, config, draw=True)
            


def run_neat(config):
    '''
    Use first line for specifc checkpoint (resume NEAT or test best AI from a checkpoint)
    '''
    ...
    p.add_reporter(neat.Checkpointer(1))  # Saves checkpoint after every generation

    '''
    Pass 50 for training AI and pass 1 to test best AI from certain checkpont
    '''
    winner = p.run(eval_genomes, 50)
    # Stores best genome to pickle file which can store an entire python object
